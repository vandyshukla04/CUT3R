#!/usr/bin/env python3
"""
Viewpoint Analyzer v5 for WildLIFT Pipeline

Interactive selection-based filmstrip reporting with user approval workflow.

Key Changes from v4:
    - Interactive mode: Shows ALL candidate frames above threshold for user approval
    - Quality threshold filtering: Only shows candidates above configurable threshold
    - NO NMS applied to candidates - user sees all options and decides
    - Vertical strip layout for PDF when few frames per orientation
    - Saves approved selections for reproducibility

Usage:
    # Interactive selection mode (default)
    python viewpoint_analyzer_v5.py --annotator_output results/zebra/scene1/corrected/ \\
        --images_dir data/zebra/scene1/images/

    # With higher quality threshold
    python viewpoint_analyzer_v5.py --annotator_output results/zebra/scene1/corrected/ \\
        --images_dir data/zebra/scene1/images/ --min_quality 0.3

    # Use saved selections (batch mode)
    python viewpoint_analyzer_v5.py --annotator_output results/zebra/scene1/corrected/ \\
        --images_dir data/zebra/scene1/images/ --use_saved_selections
"""
...
CONFIG = {
    ...
    # V5: Quality Filtering (NO NMS by default for interactive mode)
    'MIN_QUALITY_THRESHOLD': 0.15,
    'MAX_CANDIDATES_TO_SHOW': 20,  # Max candidates to show in GUI per orientation

    # PDF Layout
    'PDF_LAYOUT': 'vertical',  # 'vertical' (orientations as columns) or 'horizontal'
...
class ViewpointAnalyzer:
    ...
    def __init__(self, annotator_output_dir, images_dir=None):
        ...
        print(f"Viewpoint Analyzer v5 initialized:")
    ...
    def get_candidates_for_orientation(self, track_id: int, orientation: str,
        ...
        """
        Get ALL candidate frames for a specific orientation above quality threshold.
        NO NMS applied - user sees all options and decides.
        """
        ...
        # Limit to max_candidates (but no NMS - just top N by quality)
        return candidates[:max_candidates]

    def get_quality_statistics(self, track_id: int, frame_qualities: Dict) -> Dict:
        """Get statistics about quality scores for each orientation."""
        stats = {}
...
class InteractiveFrameSelector:
    ...
    def run_selection(self, min_quality: float = None) -> Dict[str, List[str]]:
        """Run interactive selection for all orientations."""
        if min_quality is None:
            min_quality = CONFIG['MIN_QUALITY_THRESHOLD']
        ...
        for orientation in visible_labels:
            ...
            # Load crop images for candidates
    ...
    def _show_selection_gui(self, orientation: str,
        ...
        """Show matplotlib GUI for selecting frames."""
        n_candidates = len(candidates)
        if n_candidates == 0:
            return []

        # Calculate grid layout - prefer more columns for landscape view
...
class QualityGrader:
    GRADE_COLORS = {'A': '#2ECC71', 'B': '#3498DB', 'C': '#F39C12', 'F': '#E74C3C'}
    GRADE_LABELS = {'A': 'Excellent', 'B': 'Good', 'C': 'Fair', 'F': 'Poor'}

    def grade_selections(self, selections: Dict[str, List[str]]) -> GradeResult:
        """Grade based on user selections."""
        visible_labels = CONFIG['VISIBLE_FACES']

        # Count orientations with at least 1 selection
...
# =============================================================================
# PDF REPORT GENERATOR (V5 - Vertical Strip Layout)
# =============================================================================

class PDFReportGenerator:
    """Generates filmstrip PDF with vertical strip layout."""
    ...
    def generate_report(self, track_id: int, selections: Dict[str, List[str]],
        ...
        """Generate filmstrip PDF with vertical strips (orientations as columns)."""
        if output_path is None:
            output_path = (self.analyzer.annotator_output_dir /
                          "viewpoint_analysis" / f"filmstrip_v5_track{track_id}.pdf")

        output_path.parent.mkdir(exist_ok=True)

        print(f"\n{'='*70}")
        print("GENERATING FILMSTRIP PDF (V5 - Vertical Strip Layout)")
        print(f"{'='*70}")
    ...
    def _create_vertical_filmstrip_page(self, pdf: PdfPages, track_id: int,
                                        ...
                                        grade: GradeResult):
        """
        Create vertical strip layout: each orientation is a column,
        frames stack vertically within each column.
        """
        visible_labels = CONFIG['VISIBLE_FACES']
        n_cols = len(visible_labels)

        # Find max frames per orientation
        max_frames = max(len(selections.get(l, [])) for l in visible_labels)
        max_frames = max(max_frames, 1)  # At least 1 row

        # Figure size: landscape for vertical strips
        fig = plt.figure(figsize=(11, 8.5))

        # Grid: header row + frame rows, one column per orientation
    ...
    def save_json_report(self, track_id: int, selections: Dict[str, List[str]],
                        frame_qualities: Dict,
                        output_path: Path = None) -> Path:
        """Save selections and metadata to JSON."""
        if output_path is None:
            output_path = (self.analyzer.annotator_output_dir /
                          "viewpoint_analysis" / f"selections_v5_track{track_id}.json")
...
def main():
    parser = argparse.ArgumentParser(
        description="Viewpoint Analyzer v5 - Interactive Selection-based Filmstrip",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    # Process ALL tracks with semantic labels (default)
    python viewpoint_analyzer_v5.py --annotator_output results/zebra/scene1/ \\
        --images_dir data/zebra/scene1/images/

    # Process specific track(s)
    python viewpoint_analyzer_v5.py --annotator_output results/zebra/scene1/ \\
        --images_dir data/zebra/scene1/images/ --track_id 0 1 5

    # Interactive track selection (prompts you to choose)
    python viewpoint_analyzer_v5.py --annotator_output results/zebra/scene1/ \\
        --images_dir data/zebra/scene1/images/ --select_tracks

    # With higher quality threshold (show fewer, better candidates)
    python viewpoint_analyzer_v5.py --annotator_output results/zebra/scene1/ \\
        --images_dir data/zebra/scene1/images/ --min_quality 0.3

    # Use saved selections (batch mode, regenerate PDFs)
    python viewpoint_analyzer_v5.py --annotator_output results/zebra/scene1/ \\
        --images_dir data/zebra/scene1/images/ --use_saved_selections
        """
    )
    ...
    parser.add_argument("--track_id", type=int, nargs='*', default=None,
                       help="Specific track ID(s) to process. Use without values to select interactively. Omit to process all tracks with semantic labels.")
    ...
    print("=" * 70)
    print("VIEWPOINT ANALYZER v5")
    print("Interactive Selection-based Filmstrip")
    print("=" * 70)
    ...
    if args.select_tracks or (args.track_id is not None and len(args.track_id) == 0):
        ...
        # Default: process ALL tracks with semantic labels
        tracks_to_process = analyzer.labeled_tracks
