#!/usr/bin/env python3
"""
Enhanced demo with 3D bounding boxes for instances (KITTI-style)
"""
...
def parse_dji_logs(log_file_path, frame_indices):
    """Parse DJI SRT log file to extract gimbal data for specific frames"""
    import re
    gimbal_data = {}
    
    if not os.path.exists(log_file_path):
        print(f"⚠️ DJI log file not found: {log_file_path}")
        return None
    ...
    # FIXED: Correct pattern order (rel_alt comes BEFORE gb_yaw in your SRT files)
    ...
    # Get all available FrameCnt values for debugging
    ...
    if frame_indices:
        ...
        if len(direct_matches) == 0:
            print("⚠️ No direct matches! Using ALL SRT frames instead")
            target_frames = srt_frame_indices
        else:
    ...
    # Parse gimbal data with FIXED parameter order
    ...
    # Show sample data
    if gimbal_data:
        sample_frames = sorted(gimbal_data.keys())[:3]
        print(f"📋 Sample gimbal data:")
        for frame_id in sample_frames:
            data = gimbal_data[frame_id]
            print(f"   Frame {frame_id}: yaw={data['yaw']:+6.1f}°, pitch={data['pitch']:+6.1f}°, roll={data['roll']:+6.1f}°, alt={data['altitude']:6.1f}m")
